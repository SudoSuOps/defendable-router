...
def test_worker_registration_returns_token_once_and_hashes_storage(client):
    registered = register_worker(client)
    assert registered["worker_id"].startswith("worker_")
    assert registered["worker_token"].startswith("dwrk_")
    assert registered["status"] == "registered"

    with client.app.state.testing_session_local() as db:
        worker = db.get(Worker, registered["worker_id"])
        assert worker is not None
        assert worker.auth_token_hash != registered["worker_token"]
        assert worker.auth_token_hash == hash_token(registered["worker_token"])


def test_heartbeat_with_valid_token_updates_last_heartbeat(client):
    registered = register_worker(client)
    response = client.post("/workers/heartbeat", headers=auth_headers(registered["worker_token"]), json={
        "status": "online",
        "current_jobs": 0,
        "metrics": {"gpu_util_pct": 0, "gpu_temp_c": 45}
    })
    assert response.status_code == 200
    body = response.json()
    assert body["ok"] is True
    assert body["worker_id"] == registered["worker_id"]
    assert body["next_heartbeat_seconds"] == 30
